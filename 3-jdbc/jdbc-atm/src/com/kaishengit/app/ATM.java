...
public class ATM {
	...
	private void recoverAccount() {
		...
		// 3.判断该账户是否存在
		Account acc = accountDao.findByName(name);
	...
	private void insertCard() {
		...
		currAccount = accountDao.findByName(name);
//		currAccount = bank.findByNameAndPassword(name, password);// accounts[i]

		if (currAccount != null && password.equals(currAccount.getPassword())) {
			// 判断账户状态
	...
	private void transMoney() {
		...
		// 2.校验该账户是否存在
		Account inAccount = accountDao.findByName(name);
		// 3.如果转入账户存在
		if (inAccount != null) {
			// 3.1 用户输入转账金额
